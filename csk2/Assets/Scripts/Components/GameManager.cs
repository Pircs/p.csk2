...
namespace com.QH.QPGame.Lobby
{
    ...
    public class GameManager
    {
        ...
        {
			com.QH.QPGame.GameUtils.Logger.Sys.Log("GameManager Init");
        ...
        public void LoadGameConfig()
        {
            ...
            {
				com.QH.QPGame.GameUtils.Logger.Sys.LogError("empty game config file");
                return;
            }
            ...
            foreach (var config in dataList)
            {
                ...
                foreach (var ab in config.AssetBundles)
                {
                    if (!config.Packed)
                    {
                        ...
                        if (!config.Installed)
                        {
							com.QH.QPGame.GameUtils.Logger.Sys.Log("the game is not installed.file:"+ab);
                            break;
                        }
                    }
                    ...
                    {
						com.QH.QPGame.GameUtils.Logger.Sys.Log("the game is needs update.file:" + ab);
                        config.NeedUpdate = true;
                        break;
                    }
                }
				com.QH.QPGame.GameUtils.Logger.Sys.Log("add game:" + config.ID+" Installed:"+config.Installed+" UpdateNeeds:"+config.NeedUpdate);
