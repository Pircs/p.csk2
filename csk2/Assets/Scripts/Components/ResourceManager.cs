...
//        private string downloadLoadPath = GameHelper.GetDownloadPath();
		...
		private string downloadLoadPath = string.Empty;


        public bool Initialize()
        {
			com.QH.QPGame.GameUtils.Logger.Res.Log("Res System Init");
        ...
        private IEnumerator InternalInit()
        {
                ...
                Caching.enabled )
            {
                ...
                {
			com.QH.QPGame.GameUtils.Logger.Res.Log("clean all cache and download files.PreVer:"+GameApp.GameData.InstallVersion);
        ...
        {
			com.QH.QPGame.GameUtils.Logger.Res.Log("scan files in path:" + downloadPath);
            ...
            try
            {
                ...
                {
					com.QH.QPGame.GameUtils.Logger.Res.Log("download path is not exists Path:" + downloadPath);
                    return;
                }
            ...
            {
				com.QH.QPGame.GameUtils.Logger.Res.LogException(ex);
            }
        }
        ...
        public void LoadRemoteResConfigFromString(string text)
        {
            try
            {
                ...
                foreach (var assetBundleInfo in remoteResVer)
                {
                    ...
                    {
						com.QH.QPGame.GameUtils.Logger.Res.LogError(assetBundleInfo.FileName + " 已经存在");
                        return;
                    }
        ...
        public GameOptions LoadGameOptions()
        {
            try
            {
                ...
                {
					com.QH.QPGame.GameUtils.Logger.Res.LogError("load app config file failed.");
                    return null;
                }
...
//#endif
        ...
        public void LoadResConfig()
        {
            try
            {
                ...
                if (configData == null)
                {
					com.QH.QPGame.GameUtils.Logger.Sys.LogError("load res version file failed. Text:" + text);
                    return;
                }
        ...
        public bool IsResDownloaded(string ab)
        {
			...
			com.QH.QPGame.GameUtils.Logger.UI.Log("no exists in local res config. File:" + ab);
            return false;
        }
        ...
        public bool IsResNeedsUpdate(string ab)
        {
            ...
            if (!remoteResConfig.ContainsKey(ab))
            {
				com.QH.QPGame.GameUtils.Logger.UI.Log("no exists in remote res config. File:" + ab);
                return false;
            }
        ...
        public bool IsValidABFile(AssetBundleInfo local, AssetBundleInfo remote, bool checkFile)
        {
            try
            {
                ...
                {
					com.QH.QPGame.GameUtils.Logger.UI.Log("different ab name or version. local:" + local + " remote:" + remote);
                    return false;
                }
                ...
                if (GameApp.Options.EnableVerifyAB)
                {
                    ...
                    {
						com.QH.QPGame.GameUtils.Logger.UI.Log("different ab hash");
                        return false;
                    }

                    if (FileHelper.IsFileValid(fileName, local.Hash))
                    {
						com.QH.QPGame.GameUtils.Logger.UI.Log("different ab file hash");
                        return true;
                    }
                    else
                    {
						com.QH.QPGame.GameUtils.Logger.Sys.LogWarning("same ab file:" + local + " with different md5 value");
                    }
                }
                else
                {
        ...
        public IEnumerator LoadTextFileAsync(string file)
        {
			...
			com.QH.QPGame.GameUtils.Logger.Res.Log("load text:" + file);
            ...
            if (www == null || !string.IsNullOrEmpty(www.error))
            {
                ...
                {
					com.QH.QPGame.GameUtils.Logger.Res.Log("get file text error. Error:" + www.error + " File:" + file);
                    yield break;
                }
            }
       ...
       /// <returns></returns>
        ...
        public IEnumerator LoadAssetAsyncInResources<T>(string prefab) where T : UnityEngine.Object
        {
			...
			com.QH.QPGame.GameUtils.Logger.Res.Log("asset loaded from resources. Prefab:"+prefab);
        ...
        public IEnumerator LoadLevel(string scene)
        {
            ...
            {
				com.QH.QPGame.GameUtils.Logger.Res.Log("no match config for scene which named:"+scene);
                yield break;
            }
            ...
            float useTime = Time.realtimeSinceStartup - beginReadTime;
			com.QH.QPGame.GameUtils.Logger.Res.Log("level loaded. Name:" + scene + " UseTime:" + useTime);
        }

        public bool IsSceneCached(string scene)
        {
			com.QH.QPGame.GameUtils.Logger.Res.Log("check cache, ready:" + Caching.ready + " enabled:" + Caching.enabled);
			...
			com.QH.QPGame.GameUtils.Logger.Res.Log("check cache, url:"+url+" ver:"+version);
        ...
        public bool IsCachedLessThan(long size)
        {
			...
			com.QH.QPGame.GameUtils.Logger.Res.Log("check cache, free:"+Caching.spaceFree+
        ...
        {
			com.QH.QPGame.GameUtils.Logger.Res.Log("unload level:" + scene);
