...
//			HallTransfer.Instance.cnSetRoomInfo(tempDesk);//发送桌子数据
        ...
        {
			com.QH.QPGame.GameUtils.Logger.UI.Log("NcGameRoomClick:" + roomID);
            ...
            if (item == null)
            {
				com.QH.QPGame.GameUtils.Logger.UI.LogError("no match room id:" + roomID);
                return;
            }
            ...
            {
				com.QH.QPGame.GameUtils.Logger.UI.LogError("the game needs update or download GameID:" + item.GameNameID);
                return;
            }
#endif
