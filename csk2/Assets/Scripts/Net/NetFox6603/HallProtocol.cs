...
namespace com.QH.QPGame.Services.NetFox
{
    internal class HallProtocol : Singleton<HallProtocol>
    {
        ...
        void OnLogonSuccess(Packet packet)
        {
			...
			com.QH.QPGame.GameUtils.Logger.Net.Log("User:" + loginData.szAccounts + " Loged|Money:" + loginData.lUserScore + "|Bank:" + loginData.lUserInsure);
            while (true)
            {
                string str = "";
		...
		private bool AnalysisLockAccount(Packet packet)
		{
			...
			if (packet.DataSize < dataStruct)
			{
				com.QH.QPGame.GameUtils.Logger.Net.LogError("Lock   Account  Data Error CMD_GP_LockAndUnlock!");
				return false;
			}
