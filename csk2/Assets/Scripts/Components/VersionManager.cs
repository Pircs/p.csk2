...
namespace com.QH.QPGame.Lobby
{
    ...
    public class VersionManager : MonoBehaviour
    {
        ...
        public bool Run()
        {
            ...
            Status = "获取远程程序配置文件";
			com.QH.QPGame.GameUtils.Logger.UI.Log(Status);
            return true;
        }
        ...
        private void OnDownloadRetry(int times, object obj)
        {
            Status = string.Format("下载失败 {0}/{1}，再次重试", times, 
                GlobalConst.Update.RetryTimes);
			com.QH.QPGame.GameUtils.Logger.Net.Log(Status);
        }
        ...
        private void ErrorOccurred(string error)
        {
            ...
            {
			com.QH.QPGame.GameUtils.Logger.Sys.Log("download failed. Error:" + error);
            }
        ...
        private void OnDownloadAppFile(bool b, object result, object arg3)
        {
            ...
            Status = "程序配置文件下载完成";
			com.QH.QPGame.GameUtils.Logger.Sys.Log(Status);

            forceUpdate = IsForceUpdate();

            var remoteAppVer = JsonMapper.ToObject<AppVersionDesc>(text);
            var remoteConfig = remoteAppVer.FindConfig(
                new[] { forceUpdate ? GameApp.GameData.Version : "res", "other" });
        ...
        {
			com.QH.QPGame.GameUtils.Logger.Sys.LogException(exception);
        ...
        private void OnDownloadCompleted(bool allDone, object o, object userState)
        {
			...
			com.QH.QPGame.GameUtils.Logger.Sys.Log(str);
        ...
        private void DownloadFiles(ICollection<AssetBundleInfo> files)
        {
			...
			com.QH.QPGame.GameUtils.Logger.Sys.Log("add download task. Url:" + url);
        ...
        {
			com.QH.QPGame.GameUtils.Logger.Sys.Log("add download task. GameID:" + gameID);
            ...
            if (config == null)
            {
			com.QH.QPGame.GameUtils.Logger.Sys.LogError("no match game's config for GameID:" + gameID);
                return;
            }
        ...
        public void CancelDownload(int gameID)
        {
            if (downloader != null)
            {
			com.QH.QPGame.GameUtils.Logger.Sys.Log("cancel download. GameID:" + gameID);
                downloader.CancelTask();
                ErrorOccurred("用户取消");
            }
        }
