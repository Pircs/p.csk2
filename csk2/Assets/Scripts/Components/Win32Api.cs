    ...
    public class Win32Api : Singleton<Win32Api>
    {
        ...
        public void OpenUrlInBrowser(string title, string url, int width, int height)
        {
            try
            {
				...
				com.QH.QPGame.GameUtils.Logger.Sys.Log("OpenUrlInBrowser:" + fileName);
				com.QH.QPGame.GameUtils.Logger.Sys.Log("OpenUrlInBrowser:" + strParam);

                mProcess = new Process();
                mProcess.EnableRaisingEvents = true;
				mProcess.Exited += (sender, args) => { com.QH.QPGame.GameUtils.Logger.Sys.Log("!!!!!!!!!!!!!!!!!!!!!"); mProcess = null; };
        ...
        public void CloseBrowser()
        {
            try
            {
				com.QH.QPGame.GameUtils.Logger.Sys.Log("CloseBrowser " + (mProcess != null ? mProcess.HasExited.ToString() : ""));
                if (mProcess != null)
                {
                    mProcess.Kill();
                    mProcess.Dispose();
